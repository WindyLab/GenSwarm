...
if __name__ == "__main__":
    ...
    root_directory_cap = "../workspace/comparative/cap"  # 修改为实际路径
    root_directory_metagpt = "../workspace/comparative/meta"  # 修改为实际路径
    root_directory_llm2swarm = "../workspace/comparative/llm2swarm"
    ...
    output_combined_file = "combined_success_rates.json"  # 保存最终合并数据的文件路径

    task_data_4o, wo_vlm_data, _ = process_experiment_data(root_directory_4o)
    cap_data, _, _ = process_experiment_data(root_directory_cap)
    metagpt_data, _, _ = process_experiment_data(root_directory_metagpt)
    o1_mini_data, _, _ = process_experiment_data(root_directory_o1_mini)
    llm2swarm_data, _, _ = process_experiment_data(root_directory_llm2swarm)
    ...
    cap_averages = {
        task: sum(rates) / len(rates)
        for task, rates in cap_data.items()
    }
    metagpt_averages = {
        task: sum(rates) / len(rates)
        for task, rates in metagpt_data.items()
    }
    o1_mini_averages = {
        task: sum(rates) / len(rates)
        for task, rates in o1_mini_data.items()
    }
    llm2swarm_averages = {task: sum(rates) / len(rates) for task, rates in llm2swarm_data.items()}  # ✅ 新增平均值

    print(
        f"4o 平均成功率: {task_averages_4o}\n"
        f"wo_vlm 平均成功率: {wo_vlm_averages}\n"
        f"cap 平均成功率: {cap_averages}\n"
        f"metagpt 平均成功率: {metagpt_averages}\n"
        f"o1_mini 平均成功率: {o1_mini_averages}\n"
        f"llm2swarm 平均成功率: {llm2swarm_averages}"
    ...
    cap_averages = sum(cap_averages.values()) / len(cap_averages)
    metagpt_averages = sum(metagpt_averages.values()) / len(metagpt_averages)
    o1_mini_averages = sum(o1_mini_averages.values()) / len(o1_mini_averages)
    llm2swarm_averages = sum(llm2swarm_averages.values()) / len(llm2swarm_averages)
    ...
    combined_data = {
        "ours": task_data_4o,
        "ours w/o vlm": wo_vlm_data,
        "cap": cap_data,
        "metagpt": metagpt_data,
        "llm2swarm": llm2swarm_data  # ✅ 合并数据
