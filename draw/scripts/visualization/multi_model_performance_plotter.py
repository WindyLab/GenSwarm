...
def plot_figure2(tasks_top3: List[str], methods: List[str], data_2_3: Dict[str, Dict[str, np.ndarray]], output_dir: str, group_gap=1,
                 method_gap=0.7):
    """
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "model_figure2.svg"), format="svg")
    plt.close(fig)


def plot_figure3(tasks_bot3: List[str], methods: List[str], data_2_3: Dict[str, Dict[str, np.ndarray]], output_dir: str, group_gap=1,
                 method_gap=0.7):
    """
    绘制 Figure 3 并保存。
    """
    ...
    ax.legend(handles=method_handles, loc="upper right")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "model_figure3.svg"), format="svg")
    plt.close(fig)
...
def main():
    ...
    # 从 JSON 文件加载数据
    file_path = "../../results/data/model_combined_success_rates.json"  # 文件路径
    ...
    plot_figure2(tasks_top3, methods, data_2_3, output_dir)

    # 绘制 Figure 3 (Bottom 3 tasks × 4 methods)
    plot_figure3(tasks_bot3, methods, data_2_3, output_dir)
